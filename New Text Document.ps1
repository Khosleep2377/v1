...
Get-ChildItem $ifPath -ErrorAction SilentlyContinue | ForEach-Object {
    foreach ($k in $ifVals.Keys) {
        Set-ItemProperty -Path $_.PSPath -Name $k -Value $ifVals[$k] -Type DWord -Force -ErrorAction SilentlyContinue
    }
}

Write-Host "[OK] Interfaces optimized" -ForegroundColor Green

# =========================
# 3. REGISTRY - GLOBAL TCP
# =========================
...
foreach ($k in $pVals.Keys) {
    Set-ItemProperty -Path $pPath -Name $k -Value $pVals[$k] -Type DWord -Force -ErrorAction SilentlyContinue
}

# =========================
# 4. MULTIMEDIA GAME PROFILE
# =========================
$mmPath = "HKLM:\SOFTWARE\Microsoft\Windows NT\CurrentVersion\Multimedia\SystemProfile"

Set-ItemProperty $mmPath -Name "NetworkThrottlingIndex" -Value 0xffffffff -Type DWord -Force -ErrorAction SilentlyContinue
Set-ItemProperty $mmPath -Name "SystemResponsiveness" -Value 0 -Type DWord -Force -ErrorAction SilentlyContinue

$game = "$mmPath\Tasks\Games"
if (-not (Test-Path $game)) { New-Item $game -Force | Out-Null }

Set-ItemProperty $game -Name "GPU Priority" -Value 8 -Type DWord -Force -ErrorAction SilentlyContinue
Set-ItemProperty $game -Name "Priority" -Value 6 -Type DWord -Force -ErrorAction SilentlyContinue
Set-ItemProperty $game -Name "Scheduling Category" -Value "High" -Type String -Force -ErrorAction SilentlyContinue

Write-Host "[OK] Game profile optimized" -ForegroundColor Green

# =========================
# 5. PROCESS PRIORITY (GAMES ONLY)
# =========================
$games = @(
...
foreach ($g in $games) {
    ...
    if ($p) {
        try {
            ...
            Write-Host "[HIGH] $g" -ForegroundColor Green
        } catch {}
    }
}

# =========================
# 6. POWER / SYSTEM TWEAKS
# =========================
try {
...
try {
    ...
    Write-Host "[OK] Timer tweaks applied" -ForegroundColor Green
} catch {}
...
foreach ($s in $services) {
    try {
        ...
        Write-Host "[OFF] $s" -ForegroundColor Yellow
    } catch {}
}
